...
public final class BlendImagePipeline: ImagePipelineType {
    ...
    private static func renderBlend(images: [ASS_Image], boundingRect: CGRect) -> (UnsafeMutablePointer<UInt8>, Int) {
        ...
        loop(iterations: images.count) { i in
            ...
            let red = Float((image.color >> 24) & 0xFF) / 255.0
            let green = Float((image.color >> 16) & 0xFF) / 255.0
            let blue = Float((image.color >> 8) & 0xFF) / 255.0
        ...
        loop(iterations: height) { _ in
            loop(iterations: width) { x in
                ...
                if alpha >= 1 / 255.0 {
                    result[index] = clamp(buffer[index] / alpha)
                    result[index + 1] = clamp(buffer[index + 1] / alpha)
                    result[index + 2] = clamp(buffer[index + 2] / alpha)
                    result[index + 3] = clamp(alpha)
                }
            }
            position += rowBytes
        }
